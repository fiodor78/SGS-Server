...
struct commands_leaderboard_view: symbols<int>
{
	commands_leaderboard_view(){add("info", CMD_LEADERBOARD_INFO);}
} commands_leaderboard_view_p;

struct leaderboard_gr : public grammar<leaderboard_gr>
{
	leaderboard_gr(unsigned& cmd_, map<string, string>& options_, pair<string, string>& p_)
		: cmd(cmd_),options(options_),p(p_){}
	template <typename ScannerT>
	struct definition
	{
		definition(leaderboard_gr const& self) :
		keyword_p("0-9a-zA-Z_"),
		keyword_d("a-zA-Z0-9_")
		{
			view_name = keyword_d[commands_leaderboard_view_p[assign_a(self.cmd)]];
			...
			exp1 = keyword_p("leaderboard") >> view_name >> *(( option_name[assign_a(self.p.first)] >> '=' >> option_value )[insert_at_a(self.options, self.p.first, self.p.second)]);
			expression=*(exp1);
		}
...
/***************************************************************************/
GRaportInterface &  GConsoleLeaderBoard::GetRaportInterface()
{
	GLeaderBoardManager * server_leaderboard = static_cast<GLeaderBoardManager*>(server);
	return server_leaderboard->raport_interface_local;
}
/***************************************************************************/
void GConsoleLeaderBoard::ConsoleLeaderBoard(GSocketConsole *socket,char * buf)
{
	//GLeaderBoardManager * server_leaderboard = static_cast<GLeaderBoardManager*>(server);

	PrepareBuffer(socket,buf);

	unsigned command = CMD_LEADERBOARD_NONE;
	map<string, string> options;
	pair<string, string> tmp_pair;
	leaderboard_gr gr(command,options,tmp_pair);
