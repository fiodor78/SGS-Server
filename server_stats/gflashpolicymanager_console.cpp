...
struct commands_FlashPolicy_view: symbols<int>
{
	commands_FlashPolicy_view(){add("info", CMD_FLASHPOLICY_INFO);}
} commands_FlashPolicy_view_p;

struct flashpolicy_gr : public grammar<flashpolicy_gr>
{
	flashpolicy_gr(unsigned& cmd_, map<string, string>& options_, pair<string, string>& p_)
		: cmd(cmd_),options(options_),p(p_){}
	template <typename ScannerT>
	struct definition
	{
		definition(flashpolicy_gr const& self) :
		keyword_p("0-9a-zA-Z_"),
		keyword_d("a-zA-Z0-9_")
		{
			view_name = keyword_d[commands_FlashPolicy_view_p[assign_a(self.cmd)]];
			...
			exp1 = keyword_p("flashpolicy") >> view_name >> *(( option_name[assign_a(self.p.first)] >> '=' >> option_value )[insert_at_a(self.options, self.p.first, self.p.second)]);
			expression=*(exp1);
		}
...
/***************************************************************************/
GRaportInterface &  GConsoleFlashPolicy::GetRaportInterface()
{
	GFlashPolicyManager * server_flashpolicy=static_cast<GFlashPolicyManager*>(server);
	return server_flashpolicy->raport_interface_local;
}
/***************************************************************************/
void GConsoleFlashPolicy::ConsoleFlashPolicy(GSocketConsole *socket,char * buf)
{
	//GFlashPolicyManager * server_flashpolicy = static_cast<GFlashPolicyManager*>(server);

	PrepareBuffer(socket,buf);

	unsigned command = CMD_FLASHPOLICY_NONE;
	map<string, string> options;
	pair<string, string> tmp_pair;
	flashpolicy_gr gr(command,options,tmp_pair);
